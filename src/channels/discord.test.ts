...
const loginErrorRef = vi.hoisted(() => ({ current: null as Error | null }));
...
describe('DiscordChannel', () => {
  ...
  describe('connection lifecycle', () => {
    ...
    it('rejects connect() when login fails', async () => {
      loginErrorRef.current = new Error('Used disallowed intents');
    ...
    it('enables proxy support when proxy env is configured', async () => {
  ...
  describe('proxy helpers', () => {
    it('prefers HTTPS proxy over other proxy env vars', () => {
      const proxyUrl = resolveDiscordProxyUrl({
        HTTPS_PROXY: 'http://https-proxy:7897',
        ALL_PROXY: 'socks5://all-proxy:7897',
        HTTP_PROXY: 'http://http-proxy:7897',
      });

      expect(proxyUrl).toBe('http://https-proxy:7897');
    });

    it('patches the ws module to inject an agent', () => {
      class FakeWebSocket {
        static OPEN = 1;
      ...
      patchDiscordWebSocketProxy('http://127.0.0.1:7897', wsModule as any);

      const PatchedWebSocket = wsModule.WebSocket as typeof FakeWebSocket;
      const socket = new PatchedWebSocket('wss://gateway.discord.gg', [], {
      ...
      expect(socket.options).toEqual(
        expect.objectContaining({
          handshakeTimeout: 30000,
          agent: expect.objectContaining({ kind: 'proxy-agent' }),
        }),
      );
      expect((PatchedWebSocket as any).OPEN).toBe(1);
    });
  });
