...
describe('credentials detection', () => {
  it('detects ANTHROPIC_API_KEY in env content', () => {
    ...
    const hasCredentials =
      /^(CLAUDE_CODE_OAUTH_TOKEN|ANTHROPIC_API_KEY|GITHUB_TOKEN|GH_TOKEN)=/m.test(
        content,
      );
  ...
  it('detects GITHUB_TOKEN in env content', () => {
    const content = 'GITHUB_TOKEN=gho_test123';
    const hasCredentials =
      /^(CLAUDE_CODE_OAUTH_TOKEN|ANTHROPIC_API_KEY|GITHUB_TOKEN|GH_TOKEN)=/m.test(
        content,
      );
  ...
  it('detects GH_TOKEN in env content', () => {
    const content = 'GH_TOKEN=gho_test123';
    const hasCredentials =
      /^(CLAUDE_CODE_OAUTH_TOKEN|ANTHROPIC_API_KEY|GITHUB_TOKEN|GH_TOKEN)=/m.test(
        content,
      );
